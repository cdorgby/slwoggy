...
class dedup_filter : public log_filter
{
  private:
    ...
    void cleanup_old_entries()
    {
        ...
        {
            if (it->second.last_seen < cutoff) { it = seen_messages_.erase(it); }
            else { ++it; }
        }
        last_cleanup_ = now;
    }
  ...
  public:
    ...
    void process_batch(log_buffer_base **buffers, size_t count) override
    {
        ...
        for (size_t i = 0; i < count; ++i)
        {
            ...
            if (!buffer || buffer->is_flush_marker()) { continue; }
    ...
    void shutdown() override { seen_messages_.clear(); }
  ...
  public:
    ...
    void process_batch(log_buffer_base **buffers, size_t count) override
    {
        for (size_t i = 0; i < count; ++i)
        {
            ...
            if (dist_(rng_) >= sample_rate_) { buffer->filtered_ = true; }
        }
    }
  ...
  public:
    ...
    void process_batch(log_buffer_base **buffers, size_t count) override
    {
        ...
        if (elapsed_nanos > 0)
        {
            ...
            if (tokens_in_nanos_ > max_tokens_in_nanos) { tokens_in_nanos_ = max_tokens_in_nanos; }
            last_refill_ = now;
        }
        ...
        for (size_t i = 0; i < count; ++i)
        {
            ...
            if (tokens_in_nanos_ >= NANOS_PER_SECOND) { tokens_in_nanos_ -= NANOS_PER_SECOND; }
            else
            {
...
class test_filter : public log_filter
{
  ...
  public:
    explicit test_filter(bool drop_all = false) : drop_all_(drop_all) {}
    ...
    void process_batch(log_buffer_base **buffers, size_t count) override
    {
        for (size_t i = 0; i < count; ++i)
        {
            ...
            if (drop_all_) { buffer->filtered_ = true; }
    ...
    void reset() override { processed_count_.store(0, std::memory_order_relaxed); }

    size_t get_processed_count() const { return processed_count_.load(std::memory_order_relaxed); }
