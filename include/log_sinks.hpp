...
inline std::shared_ptr<log_sink> make_stdout_sink(Filter&& filter = {})
{
    if constexpr (std::is_same_v<std::decay_t<Filter>, no_filter>)
        ...
        return std::make_shared<log_sink>(raw_formatter{true, true}, file_writer{STDOUT_FILENO}, std::forward<Filter>(filter));
}
...
inline std::shared_ptr<log_sink> make_raw_file_sink(const std::string_view &filename, rotate_policy policy = {}, Filter&& filter = {})
{
    if constexpr (std::is_same_v<std::decay_t<Filter>, no_filter>)
        ...
        return std::make_shared<log_sink>(raw_formatter{false, true}, file_writer{std::string(filename), policy}, std::forward<Filter>(filter));
}
...
inline std::shared_ptr<log_sink> make_writev_file_sink(const std::string_view &filename, rotate_policy policy = {}, Filter&& filter = {})
{
    if constexpr (std::is_same_v<std::decay_t<Filter>, no_filter>)
        ...
        return std::make_shared<log_sink>(raw_formatter{false, true}, writev_file_writer{std::string(filename), policy}, std::forward<Filter>(filter));
}
...
inline std::shared_ptr<log_sink> make_json_sink(const std::string_view &filename, rotate_policy policy = {}, Filter&& filter = {})
{
    if constexpr (std::is_same_v<std::decay_t<Filter>, no_filter>)
        ...
        return std::make_shared<log_sink>(taocpp_json_formatter{false, true}, file_writer{std::string(filename), policy}, std::forward<Filter>(filter));
}
