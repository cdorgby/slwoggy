...
void print_metrics() {
    ...
    if (metrics.enospc_deletions_raw > 0 || metrics.enospc_deletions_gz > 0) {
        std::cout << "  ENOSPC deletions: raw=" << metrics.enospc_deletions_raw.load()
                  << " gz=" << metrics.enospc_deletions_gz.load()
                  << " bytes=" << metrics.enospc_deleted_bytes.load() << "\n";
    }
...
void logging_worker(int thread_id, int messages_per_second) {
    ...
    while (!stop_threads.load()) {
        ...
        LOG(info) << "Thread-" << thread_id 
                  << " msg#" << total_messages.fetch_add(1)
                  << " " << padding;
...
void demo_size_rotation() {
    ...
    for (int i = 0; i < 1000; ++i) {
        LOG(info) << "Size rotation test message " << i 
...
void demo_combined_rotation() {
    ...
    for (int i = 0; i < 500; ++i) {
        LOG(info) << "Burst message " << i << " with padding: " << std::string(100, 'B');
    }
...
void demo_retention_policies() {
    ...
    for (int rotation = 0; rotation < 6; ++rotation) {
        ...
        for (int i = 0; i < 150; ++i) {
            LOG(info) << "Retention test rotation " << rotation 
                      << " message " << i << " " << std::string(80, 'R');
        }
        std::this_thread::sleep_for(100ms);
    }
...
void demo_multithreaded() {
    ...
    auto sink = std::make_shared<log_sink>(
        raw_formatter{},
        writev_file_writer(log_file, policy)
    );
    ...
    for (int i = 0; i < 4; ++i) {
        workers.emplace_back(logging_worker, i, 100);
    }
    ...
    while (std::chrono::steady_clock::now() - start < 10s) {
        ...
        auto elapsed = std::chrono::duration_cast<std::chrono::seconds>(
            std::chrono::steady_clock::now() - start).count();
        std::cout << "  Progress: " << elapsed << "s, Messages: " 
                  << total_messages.load() << "\n";
    }
    ...
    if (duration_sec > 0) {
        std::cout << "  Throughput: " << (total_messages.load() / duration_sec) 
                  << " msg/sec\n";
    }
...
int main(int argc, char* argv[]) {
    ...
    if (argc > 1) {
        ...
        if (demo == "1" || demo == "size") {
            demo_size_rotation();
        } else if (demo == "2" || demo == "time") {
            demo_time_rotation();
        } else if (demo == "3" || demo == "combined") {
            demo_combined_rotation();
        } else if (demo == "4" || demo == "retention") {
            demo_retention_policies();
        } else if (demo == "5" || demo == "multithread") {
            demo_multithreaded();
        } else if (demo == "6" || demo == "enospc") {
